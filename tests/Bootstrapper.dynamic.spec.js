...
describe('Dynamic Bootstrapper Disabled', () => {
	...
	// -- MOCH CONTAINER SET UP -- //
	const container = new Container();
	container.register('IMochComponent2', MochComponent);
	// --------------------------- //
	...
	beforeEach((done) => {
		...
		window.setTimeout(() => {
			node = document.createElement('div');
			node.innerHTML =
				'<div data-component="IMochComponent2"></div>';
	...
	it('should render after manual update', () => {
		// Manual update
		app.update(node);
